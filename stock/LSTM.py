...
def openfile(filename):
    ...
    df=pd.read_csv(f, index_col='日期', parse_dates=True).sort_index()   #读入股票数据
    df = df.drop(df[df.开盘价==0].index)
    # df=pd.read_csv(f)
    # print(df)
...
def train_lstm(batch_size=80,time_step=15,train_begin=100,train_end=2600):
    ...
    with tf.Session() as sess:
        ...
        for i in range(1000):
            ...
            print(i,loss_)
            if i % 200==0:
                print("保存模型：",saver.save(sess,checkpoint_dir+stockname,global_step=i))
